...
cleanline_root = resolve_toolbox_root_impl(cfg, "cleanline");

if strlength(cleanline_root) > 0
    if exist(char(cleanline_root), 'dir') == 7
        % hlp_varargin2struct liegt in external/.../helpers -> daher genpath
        addpath(genpath(char(cleanline_root)));
    end
end
...
if isfield(cfg, 'subjects') && isfield(cfg.subjects, 'list') && ~isempty(cfg.subjects.list)
    ...
    if use_step01_source_discovery
        ...
        dir_info = dir(fullfile(source_root, '**', '*.vhdr')); %LL search for file in sub-folders
...
session_label = ""; %LL without session label
...
function [EEG, did_apply] = apply_pop_cleanline_to_subset_impl(EEG, subset_indices, step_cfg, helpers, label_for_log)
did_apply = false;

if exist('pop_cleanline','file') ~= 2 || exist('hlp_varargin2struct','file') ~= 2
    did_apply = false;
    return;
end
...
try
    ...
    if isempty(freqs)
        did_apply = false; %true; %LL debugging
        return;
    end
    ...
    did_apply = true;
        % nach erfolgreichem pop_cleanline:
        if isstruct(helpers) && isfield(helpers,'log_msg_default') && ~isempty(label_for_log)
            helpers.log_msg_default('%s: Cleanline applied successfully (did_apply=true).', label_for_log);
        end
catch me
    EEG.data = original_data;
    did_apply = false;
    if isstruct(helpers) && isfield(helpers,'log_msg_default') && ~isempty(label_for_log)
    helpers.log_msg_default('%s: Cleanline skipped/failed (did_apply=false): %s', label_for_log, me.message);
    end
    warning('apply_pop_cleanline_to_subset failed: %s', me.message);
      try
        if nargin >= 5 && isstruct(helpers) && isfield(helpers, 'log_msg_default') && ~isempty(label_for_log)
            helpers.log_msg_default('%s: Cleanline failed: %s', label_for_log, me.message);
        end
    catch
        % fallback: nichts tun, warning bleibt schon da
    end
end
...
%LL fix --- FIX: protect pop_rejepoch against ERPLAB/EEGLAB internal empty crashes ---
bad_epochs = bad_epochs(:);

% (optional aber sinnvoll) clamp auf gültige Trial-Indizes
bad_epochs = bad_epochs(bad_epochs >= 1 & bad_epochs <= EEG_work.trials);

if isempty(bad_epochs)
    % nothing to reject after clamp -> keep
    info.n_kept = EEG_work.trials;
    if ~isfield(EEG_work, 'etc') || isempty(EEG_work.etc)
        EEG_work.etc = struct();
    end
    ...
    EEG_out = EEG_work;
    helpers.log_msg_default( ...
        'ERPLAB rejection: %s | %s | bad_epochs empty after clamp (trials=%d) -> skip pop_rejepoch', ...
        char(string(subj_label)), char(string(run_label)), EEG_work.trials);
    return;
end
...
try
    ...
    helpers.log_msg_default( ...
        'ERPLAB rejection pop_rejepoch FAILED: %s | trials=%d | bad_epochs=%d -> keep un-rejected epochs', ...
        me_pop.message, EEG_work.trials, numel(bad_epochs));

    % fallback: keep the dataset as-is (with marks but without removing epochs)
    % (wir setzen info entsprechend)
    info.n_rejected = 0;
    info.rejected_epochs = [];
    info.n_kept = EEG_work.trials;
