...
% EEG_PIPELINE_HELPERS
%
% PURPOSE
%   Return helper function handles used across the EEG pipeline.
%
% INPUT
%   default_log_file : log file used by helpers.log_msg_default(...)
%
% OUTPUT
%   helpers : struct with helper handles
...
function mode = detect_env_mode_impl(varargin) %#ok<INUSD>
mode = lower(strtrim(string(get_env_first_nonempty_impl({'EEG_PIPELINE_ENV_MODE'}))));

if any(mode == ["pc","server","hpc"])
    return;
end

machine_kind = detect_machine_kind_impl();
...
function machine_kind = detect_machine_kind_impl(varargin) %#ok<INUSD>
machine_kind = "unknown";

override = lower(strtrim(string(get_env_first_nonempty_impl({'EEG_PIPELINE_MACHINE_KIND'}))));
...
if isempty(sub_ids)
    if use_min
        ...
        error('No subjects remain after filtering.');
    end
end
end
...
paths.session_label   = string(cfg.bids.session_label);
paths.bids_task_label = string(cfg.bids.task_label);

paths.bids_sub_dir = fullfile(paths.bids_root, paths.subj_label);
paths.bids_ses_dir = fullfile(paths.bids_sub_dir, "ses-" + paths.session_label);
...
for s = 1:numel(step_names)
    ...
    if triggered && isfield(info, 'policy') && ~isempty(info.policy)
        info.run = true;
...
step_folders = normalize_step_folder_input_impl(get_step_subject_folder_impl(paths, step_name));

for i = 1:numel(step_folders)
    step_folder = step_folders{i};
...
% OVERWRITE / RERUN
% =========================================================================
function folder_list = normalize_step_folder_input_impl(step_folder)
if ischar(step_folder) || isstring(step_folder)
    folder_list = cellstr(string(step_folder));
elseif iscell(step_folder)
    folder_list = step_folder;
else
    error('step folder must be char, string, or cell array of folders.');
end
...
for i = 1:numel(folder_list)
    this_folder = char(string(folder_list{i}));
    if ~isempty(strtrim(this_folder))
        tmp{end+1,1} = this_folder; %#ok<AGROW>
    end
end
...
function [do_run, reason, info] = inspect_single_step_folder_impl(step_folder, policy)
info = struct();
info.exists   = false;
info.datenum  = NaN;
info.is_empty = true;
...
if ~isempty(real_info)
    info.datenum = min([real_info.datenum]);
end
...
datenum_vec   = nan(size(out_files));
resolved_files = cell(size(out_files));
...
switch string(overwrite_mode)
    ...
    case "if_older_than"
        ...
        if any(isnan(datenum_vec))
            do_run = true;
            reason = "at least one output has no valid file date -> regenerate";
        elseif any(datenum_vec < cutoff_datenum)
            do_run = true;
            reason = "at least one output is older than cutoff -> regenerate";
        else
            do_run = false;
            reason = "all outputs are newer than cutoff -> skip";
        end
...
folder_list = normalize_step_folder_input_impl(step_folder);

if numel(folder_list) == 1
    [do_run, reason, info] = inspect_single_step_folder_impl(folder_list{1}, policy);
    return;
end

per_folder = repmat(struct( ...
    'path', '', ...
    'exists', false, ...
    'datenum', NaN, ...
    'is_empty', true), numel(folder_list), 1);

folder_do_run  = false(numel(folder_list), 1);
folder_reason  = strings(numel(folder_list), 1);

for i = 1:numel(folder_list)
    [folder_do_run(i), folder_reason(i), tmp_info] = ...
        inspect_single_step_folder_impl(folder_list{i}, policy);

    tmp_info.path = folder_list{i};
    per_folder(i) = tmp_info;
end

info = struct();
info.exists   = any([per_folder.exists]);
info.is_empty = ~any(~[per_folder.is_empty]);
info.folders  = folder_list(:);
info.per_folder = per_folder;

all_dn = [per_folder.datenum];
valid_dn = all_dn(~isnan(all_dn));
if isempty(valid_dn)
    info.datenum = NaN;
else
    info.datenum = min(valid_dn);
end

do_run = any(folder_do_run);

if do_run
    active_reasons = folder_reason(folder_do_run);
    reason = char("multi-folder step: " + strjoin(active_reasons, " | "));
else
    reason = "all output folders satisfy policy -> skip";
end
end
...
switch step_name
    ...
    case 'prep_03_until_ica'
        step_folder = { ...
            paths.prep_03_out_dir_until_ica, ...
            paths.prep_03_out_dir_for_ica};
...
switch reference_mode
    ...
    case "avg"
        ...
        EEG_out = helpers.append_eeg_comment(EEG_out, ...
            'prep_06_epoching: average reference applied (EEG-only)');
...
for k = 1:n
    ...
    if strcmpi(t, 'boundary')
        continue;
    end
    tokens(end+1,1)  = string(t); %#ok<AGROW>
    times_s(end+1,1) = double(EEG.event(k).latency) / double(EEG.srate); %#ok<AGROW>
end
end
...
EEG = set_channel_type_by_labels_impl(EEG, step_cfg.eog_channel_labels,     'EOG');
EEG = set_channel_type_by_labels_impl(EEG, step_cfg.scr_channel_labels,     'SCR');
EEG = set_channel_type_by_labels_impl(EEG, step_cfg.startle_channel_labels, 'Startle');
EEG = set_channel_type_by_labels_impl(EEG, step_cfg.ekg_channel_labels,     'EKG');

EEG = eeg_checkset(EEG);
end

function EEG = set_channel_type_by_labels_impl(EEG, label_list, type_name)
...
for i = 1:numel(label_list)
    idx = find(strcmpi(all_labels, label_list{i}));
    for j = 1:numel(idx)
...
[idx_eeg, ~, ~] = get_channel_indices_by_type_impl(EEG);
if isempty(idx_eeg)
    return;
end
...
rep = struct();
rep.last_match_eeg_idx = last_match_eeg_idx;
...
rep.eeg_ctx_j1 = max(1, last_match_eeg_idx - 10);
rep.eeg_ctx_j2 = min(numel(eeg_tokens), last_match_eeg_idx + 30);
...
bids_base = char(string(bids_base));
